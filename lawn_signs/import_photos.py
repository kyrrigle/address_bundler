...
def import_photos(
    ...
    if not (0 <= fuzzy_threshold <= 100):
        raise ValueError(
            f"Fuzzy threshold must be between 0 and 100, got: {fuzzy_threshold}"
        )
    ...
    with open(csv_file, "r", newline="", encoding="utf-8") as file:
        ...
        if name_column not in reader.fieldnames:
            raise ValueError(
                f"Name column '{name_column}' not found in CSV. Available columns: {list(reader.fieldnames)}"
            )

        if filename_column not in reader.fieldnames:
            raise ValueError(
                f"Filename column '{filename_column}' not found in CSV. Available columns: {list(reader.fieldnames)}"
            )
        ...
        for row_num, row in enumerate(
            ...
            if match_result.match_type == "ambiguous":
                ambiguous_matches.append(
                    f"Row {row_num}: Multiple fuzzy matches for '{student_name}' - skipping"
                )
                continue
            elif match_result.match_type == "fuzzy":
                fuzzy_matches.append(
                    f"Row {row_num}: Fuzzy match: '{student_name}' → '{match_result.matched_name}' (score: {match_result.score})"
                )
            ...
            if filename:
                ...
                if image_name is None:
                    print(
                        f"Warning: Photo file '{filename}' not found for student '{student_name}'"
                    )

            # Update or create student record
            update_student_record(
                match_result, first_name, last_name, image_name, student_name
            )
            processed_count += 1
...
def find_matching_student(
    first_name: str, last_name: str, fuzzy_threshold: int
) -> MatchResult:
    """
    ...
    if len(fuzzy_matches) == 0:
        ...
        return MatchResult(
            student=student, match_type="fuzzy", score=score, matched_name=db_name
        )
    else:
...
def process_photo(
    filename: str, photos_directory: str, originals_dir: str
) -> Optional[str]:
    """
...
def update_student_record(
    ...
    if match_result.student is not None:
        ...
        if match_result.match_type == "fuzzy":
            ...
            print(
                f"Updated name and photo for student: '{match_result.matched_name}' → '{original_name}'"
            )
        else:
