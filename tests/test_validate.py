...
class TestValidateStudentImage:
    ...
    def test_no_image_name(self):
        ...
        result = validate_student_image(
            self.student, self.temp_dir, self.min_resolution
        )
...
class TestValidateStudentImages:
    ...
    def test_validate_mixed_students(self, mock_select, mock_get_project, capsys):
        ...
        assert (
            "INVALID: Bob Johnson - missing.jpg - Image file not found" in captured.out
        )

    @patch("lawn_signs.validate.get_project")
    @patch("lawn_signs.validate.Student.select")
    def test_validate_with_custom_resolution(
        self, mock_select, mock_get_project, capsys
    ):
        """Test validation with custom minimum resolution."""
        # Mock project
        mock_project = Mock()
    ...
    def test_validation_exception_handling(
        self, mock_validate, mock_select, mock_get_project, capsys
    ):
