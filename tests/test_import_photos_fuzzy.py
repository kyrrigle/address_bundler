...
class TestImportPhotosIntegration:
    ...
    def test_invalid_fuzzy_threshold(self, mock_student_class, mock_get_project):
        ...
        with pytest.raises(
            ValueError, match="Fuzzy threshold must be between 0 and 100"
        ):
    ...
    def test_fuzzy_match_reporting(
        self, mock_print, mock_student_class, mock_get_project
    ):
    ...
    @patch("lawn_signs.import_photos.Student")
    @patch("builtins.print")
    def test_ambiguous_match_handling(
        self, mock_print, mock_student_class, mock_get_project
    ):
        ...
        printed_output = [call[0][0] for call in mock_print.call_args_list]
        ambiguous_reports = [
            msg for msg in printed_output if "Multiple fuzzy matches" in msg
        ]
    ...
    def test_common_nickname_variations(self, mock_student_class):
        ...
        for import_first, db_first, last_name in test_cases:
            ...
            assert result.match_type in [
                "fuzzy",
                "exact",
            ], f"Failed to match {import_first} with {db_first}"
            if result.match_type == "fuzzy":
                assert (
                    result.score >= 70
                ), f"Score too low for {import_first} → {db_first}: {result.score}"
