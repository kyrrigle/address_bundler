...
def validate_student_images(min_resolution: int = 1000000) -> None:
    ...
    for student in students:
        validation_result = validate_student_image(
            student, originals_dir, min_resolution
        )
        ...
        if validation_result.is_valid:
            ...
            print(
                f"INVALID: {student.first_name} {student.last_name} - {student.image_name} - {validation_result.reason}"
            )
...
def validate_student_image(
    student: Student, originals_dir: str, min_resolution: int
) -> ValidationResult:
    """
        ...
        with Image.open(image_path) as img:
            ...
            if total_pixels < min_resolution:
                return ValidationResult(
                    False,
                    f"Resolution too low: {width}x{height} ({total_pixels} pixels, minimum {min_resolution})",
                )
